...
class Display:
    def __init__(self, grd: Grid, scale: float = 10.0, border: int = 3):
        ...
        self.width, self.height = (
            self.scale * self.slope_scale * self.grd.size * 2.0,
            self.scale * self.grd.size * 2.0,
        )
    ...
    def get_display_coords(cls, crd: Coord) -> tuple[float, float]:
        return (crd.y - crd.x, 2 * crd.z - (crd.x + crd.y))
    ...
    def paint_block_side(self, block: Block, side: int, color: str) -> None:
        ...
        corner_coords = [
            self.get_pixel_coords(self.get_display_coords(crd)) for crd in corners
        ]
    ...
    def place_tri(self, crd: Coord, side: bool, color: str):
        ...
        ref_x, ref_y = (
            self.scale * self.slope_scale * (crd.y - crd.x),
            self.scale * (crd.z - (crd.x + crd.y) / 2.0),
        )
        ref_x += 300
        ref_y -= 300
        self.cnv.create_polygon(
    ...
    def place_block(self, crd: Coord):
        ref_x, ref_y = (
            self.scale * self.slope_scale * (crd.y - crd.x),
            self.height - self.scale * (crd.z - (crd.x + crd.y) / 2.0),
        )
        ref_x += 300
        ref_y += 300
