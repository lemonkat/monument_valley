...
class Coord:
	...
	def is_unit(self) -> bool:
		return (
			(self.x == 1 and self.y == 0 and self.z == 0) or 
			(self.x == -1 and self.y == 0 and self.z == 0) or 
			(self.x == 0 and self.y == 1 and self.z == 0) or 
			(self.x == 0 and self.y == -1 and self.z == 0) or 
			(self.x == 0 and self.y == 0 and self.z == 1) or 
			(self.x == 0 and self.y == 0 and self.z == -1)
		)
	...
	def as_lst(self) -> list[list[list[int]]]:
		return [
			[
				[
					self.weight if (
						Coord.is_between(self.lower, Coord(x, y, z), self.upper)
					) else 0
...
class Door:
	...
	@classmethod
	def pair(a_crd:Coord,  a_facing:int, b_crd:Coord, b_facing:int, gm:"Game") -> tuple["Door"]:
		door_a, door_b = Door(a_crd, a_facing, None, gm), Door(b_crd, b_facing, None, gm)
